...
{
	public class LogOutput : Gtk.ScrolledWindow,ITaskOutput
	{
		private ListStore outputModel = new ListStore(typeof(string), typeof(string), typeof(string), typeof(string));
		...
		public LogOutput()
		{
			...
			TreeViewColumn tvcName = new TreeViewColumn (MainClass.Languages.Translate("type"),  fileNameRenderer, "text", 0);
			tvcName.MinWidth = 100;
			tvcName.MaxWidth = 100;
			//tvcName.Resizable = true;
			treeView.AppendColumn(tvcName);
			TreeViewColumn tvcState = new TreeViewColumn (MainClass.Languages.Translate("time_stamp"),  fileNameRenderer, "text", 1);
			tvcState.MinWidth = 100;
			tvcState.MaxWidth = 100;
			//tvcState.Resizable = true;
			treeView.AppendColumn(tvcState);
			TreeViewColumn tvcMessage = new TreeViewColumn ( MainClass.Languages.Translate("message"),  fileNameRenderer, "text", 2);
			tvcMessage.MinWidth = 100;
			tvcMessage.Resizable = true;
			treeView.AppendColumn(tvcMessage);

			treeView.Columns[0].SetCellDataFunc(fileNameRenderer, new Gtk.TreeCellDataFunc(RenderFileNme));
			...
			try {
				...
				if (task.errors != null) {
					...
					foreach (TaskMessage tm in task.errors)
						iter = outputModel.AppendValues(tm.File, tm.Line, tm.Message);
				}
		...
		{
			public abstract void Execute(LogOutput pad);
		}
		...
		private class QueuedTextWrite : QueuedUpdate
		{
			...
			public override void Execute(LogOutput pad)
			{
			...
			{
				Console.WriteLine("ERROR Write  LogOutput");
				//Text.Append(s);
